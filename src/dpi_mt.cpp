...
class FastPath {
public:
    FastPath(int id, Rules* rules, Stats* stats, DPI::SNILogger* sni_logger,
             TSQueue<Packet>* output_queue)
        : id_(id), rules_(rules), stats_(stats), sni_logger_(sni_logger),
...
public:
    struct Config {
        int num_lbs = 2;
        int fps_per_lb = 2;
        std::string sni_log_path = "sni_counts.csv";
    };
    
    DPIEngine(const Config& cfg) : config_(cfg), sni_logger_(cfg.sni_log_path) {
        ...
        for (int i = 0; i < total_fps; i++) {
            fps_.push_back(std::make_unique<FastPath>(i, &rules_, &stats_, &sni_logger_, &output_queue_));
        }
    ...
    bool process(const std::string& input_file, const std::string& output_file) {
        ...
        // Make sure every domain seen this run is on disk before reporting
        sni_logger_.flushNow();
