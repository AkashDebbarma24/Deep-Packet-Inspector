...
Beyond printing detected domains to the console, the multi-threaded engine can also log every
domain it sees to a CSV file, with a running count and the last time each one was observed.
This is handled by `SNILogger` (`include/sni_logger.h`, `src/sni_logger.cpp`).

### What Gets Logged

Every time `FastPath::classifyFlow()` identifies a domain — via TLS SNI or an HTTP `Host`
header — it's recorded with the packet's own capture timestamp:
...
if (sni) {
    ...
    auto ts = std::chrono::system_clock::time_point(
        std::chrono::seconds(pkt.ts_sec) +
        std::chrono::microseconds(pkt.ts_usec));
    sni_logger_->record(flow.sni, ts);
    return;
}
```

Each flow is classified once, so each domain is recorded once per **flow**, not once per packet.

### Ownership and Thread-Safety

`DPIEngine` owns a single `SNILogger` instance and shares it with every `FastPath` worker via
pointer — the same pattern already used for `Rules` and `Stats`:

```
DPIEngine
 ├── sni_logger_        (one instance, owns a background flush thread)
 ├── fps_[0] ─┐
 ├── fps_[1] ─┼── each holds a DPI::SNILogger* pointing at sni_logger_
 ├── fps_[2] ─┤
 └── fps_[3] ─┘
```

Two design choices keep this safe under concurrent access from all `FastPath` threads:

- **A single mutex guards the in-memory map.** `record()` only holds the lock for the few
  nanoseconds it takes to increment a counter, which is cheap enough that a single shared mutex
  outperforms more complex per-domain locking at this scale.
- **Flushing to disk never blocks `record()`.** The background thread swaps the live map out for
  an empty one under lock, then writes the swapped-out copy to disk with no lock held — so worker
  threads can keep logging domains the entire time the CSV write is happening.

The background thread sleeps on a `std::condition_variable` (30 seconds by default) rather than
a plain `sleep_for`, so it can be woken immediately on shutdown instead of waiting out the full
interval. `DPIEngine::process()` calls `sni_logger_.flushNow()` right before `printReport()`, so
the CSV and the console's "Detected Domains/SNIs" section always agree.
...
The file opens in append mode, with a header row written once on first creation. Because of the
append behavior, delete `sni_counts.csv` between runs if you want each run's counts to start
fresh rather than accumulate:

```bash
rm sni_counts.csv        # macOS/Linux
del sni_counts.csv       # Windows
```

### Building

`sni_logger.cpp` isn't yet wired into `CMakeLists.txt`, so add it to the manual build command:
...
- **Counts are per-flush, not cumulative for the whole run.** Each flush swaps the map out
  entirely, so a CSV row reflects activity since the *previous* flush, not a running total across
  the whole program's lifetime. Short captures that finish inside one 30-second window aren't
  affected — everything lands in the final flush. For long-running captures, sum `count` across
  appended rows per domain if you want a true cumulative total.
- **Unbounded map growth.** There's no eviction policy yet, so a busy or adversarial capture with
  many unique domains can grow the in-memory map without bound between flushes.
